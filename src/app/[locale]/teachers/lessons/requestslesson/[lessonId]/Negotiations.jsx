...
export default function Negotiations({ threadId ,onAccept ,onReject }) {
    ...
    /* ================= Helper ================= */
    const findMessageId = (data, type) => {
        const msg = msgList.find(m => {
            if (type === "accepted") {
                return (
                    m.price === data.price &&
                    m.sender._id === data.acceptedBy
                )
            }
    ...
    useEffect(() => {
        ...
        socket.on("offerAccepted", (data) => {
            ...
            const id = data.messageId ?? findMessageId(data, "accepted")
        ...
        socket.on("offerRejected", (data) => {
            ...
            const id = data.messageId ?? findMessageId(data, "rejected")
        ...
        {showAcceptedToast && (
            <div className={styles.acceptedToast}>
                <div className={styles.toastBox}>
                    ...
                    <h4>{t("offerAccepted")}</h4>
                    <p>{t("offerAcceptedDescription")}</p>

                    <button
                        className={styles.homeBtn}
                        onClick={() => {
                            setShowAcceptedToast(false)
                            router.push(`/${locale}/teachers/home`);
                        }}
                    >
                        {t("goHome")}
                    </button>
