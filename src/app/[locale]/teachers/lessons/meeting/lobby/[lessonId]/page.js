...
export default function LobbyPage() {
    ...
    const getMeeting = async () => {
      try {
        ...
        if (process.env.NODE_ENV === "development" && payload) {
          const tok = pickTeacherKitToken(payload);
  ...
  const handleJoin = () => {
    ...
    const token = pickTeacherKitToken(meetingData);
    ...
    try {
      ...
      router.push(`/${locale}/teachers/lessons/meeting/${meetingId}`);
    } catch {
      const encodedToken = encodeURIComponent(token);
      router.push(
        `/${locale}/teachers/lessons/meeting/${meetingId}?token=${encodedToken}`
      );
    }
  };
